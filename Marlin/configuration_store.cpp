  ...
  bool MarlinSettings::save() {
    ...
    EEPROM_WRITE(planner.min_travel_feedrate_mm_s);
      ...
      const float planner_max_jerk[] = {
          ...
          float(DEFAULT_AJERK), float(DEFAULT_BJERK), float(DEFAULT_CJERK), float(DEFAULT_DJERK), float(DEFAULT_EJERK), float(DEFAULT_JJERK), float(DEFAULT_HJERK)
        #else
          float(DEFAULT_XJERK), float(DEFAULT_YJERK), float(DEFAULT_ZJERK), float(DEFAULT_EJERK), float(DEFAULT_JJERK), float(DEFAULT_HJERK)
        #endif
      };
...
void MarlinSettings::reset() {
  ...
  planner.min_travel_feedrate_mm_s = DEFAULT_MINTRAVELFEEDRATE;
	...
	planner.max_jerk[H_AXIS] = DEFAULT_HJERK;
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    CONFIG_ECHO_START;
	  ...
	  SERIAL_ECHOPAIR(" J", VOLUMETRIC_UNIT(planner.axis_steps_per_mm[J_AXIS]));
	  SERIAL_ECHOPAIR(" H", VOLUMETRIC_UNIT(planner.axis_steps_per_mm[H_AXIS]));
    #endif
    SERIAL_EOL();
    ...
    CONFIG_ECHO_START;
	  ...
	  SERIAL_ECHOPAIR(" J", VOLUMETRIC_UNIT(planner.max_feedrate_mm_s[J_AXIS]));
	  SERIAL_ECHOPAIR(" H", VOLUMETRIC_UNIT(planner.max_feedrate_mm_s[H_AXIS]));
    #endif
    SERIAL_EOL();
    #if ENABLED(DISTINCT_E_FACTORS)
      CONFIG_ECHO_START;
    ...
    CONFIG_ECHO_START;
	  ...
	  SERIAL_ECHOPAIR(" J", VOLUMETRIC_UNIT(planner.max_acceleration_mm_per_s2[J_AXIS]));
	  SERIAL_ECHOPAIR(" H", VOLUMETRIC_UNIT(planner.max_acceleration_mm_per_s2[H_AXIS]));
    #endif
    SERIAL_EOL();
    #if ENABLED(DISTINCT_E_FACTORS)
      CONFIG_ECHO_START;
    ...
    if (!forReplay) {
      ...
      SERIAL_ECHOPGM("Advanced: Q<min_segment_time_us> S<min_feedrate> T<min_travel_feedrate>");
		...
		SERIAL_ECHOPGM(" J<max_j_jerk>");
		SERIAL_ECHOPGM(" H<max_h_jerk>");
    ...
    SERIAL_ECHOPAIR(" T", LINEAR_UNIT(planner.min_travel_feedrate_mm_s));
	  ...
	  SERIAL_ECHOPAIR(" J", LINEAR_UNIT(planner.max_jerk[J_AXIS]));
	  SERIAL_ECHOPAIR(" H", LINEAR_UNIT(planner.max_jerk[H_AXIS]));
