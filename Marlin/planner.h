...
class Planner {
  public:
	...
	static float j_factor[EXTRUDERS];               // The flow percentage and volumetric multiplier combine to scale E movement
	static float h_factor[EXTRUDERS];               // The flow percentage and volumetric multiplier combine to scale E movement
  ...
  public:
	...
	FORCE_INLINE static void refresh_j_factor(const uint8_t j) {
		e_factor[j] = (flow_percentage[j] * 0.01f
	#if DISABLED(NO_VOLUMETRICS)
			* volumetric_multiplier[j]
	#endif
			);
	}

	FORCE_INLINE static void refresh_h_factor(const uint8_t h) {
		e_factor[h] = (flow_percentage[h] * 0.01f
	#if DISABLED(NO_VOLUMETRICS)
			* volumetric_multiplier[h]
	#endif
			);
	}
    ...
    FORCE_INLINE static bool buffer_line(ARG_X, ARG_Y, ARG_Z,
      ...
      return buffer_segment(rx, ry, rz,
          ...
          re1,
        #endif
        e, j, h, fr_mm_s, extruder, millimeters
      );
    }
     ...
     *  millimeters  - the length of the movement, if known
     */
    FORCE_INLINE static bool buffer_line_kinematic(const float (&cart)[XYZEJH], const float &fr_mm_s, const uint8_t extruder, const float millimeters = 0.0) {
      #if PLANNER_LEVELING
      ...
      #else
        return buffer_segment(raw[X_AXIS], raw[Y_AXIS], raw[Z_AXIS], cart[E_CART], cart[J_CART], cart[H_CART], fr_mm_s, extruder, millimeters);
      #endif
    }
    ...
    static void set_position_mm_kinematic(const float (&cart)[XYZEJH]);
    static void set_position_mm(const AxisEnum axis, const float &v);

    FORCE_INLINE static void set_z_position_mm(const float &z) { set_position_mm(Z_AXIS, z); }
    FORCE_INLINE static void set_e_position_mm(const float &e) { set_position_mm(E_AXIS, e); }
	FORCE_INLINE static void set_j_position_mm(const float &j) { set_position_mm(J_AXIS, j); }
	FORCE_INLINE static void set_h_position_mm(const float &h) { set_position_mm(H_AXIS, h); }
