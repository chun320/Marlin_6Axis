 ...
 */
float current_position[XYZEJH] = { 0, 0, 0, 0, 0, 0 };
 ...
 */
float destination[XYZEJH] = { 0, 0, 0, 0, 0, 0 };
...
bool axis_relative_modes[XYZEJH] = AXIS_RELATIVE_MODES;
...
const char axis_codes[XYZEJH] = { 'X', 'Y', 'Z', 'E', 'J', 'H' };
...
void set_current_from_steppers_for_axis(const AxisEnum axis);
  ...
  void plan_arc(const float (&cart)[XYZEJH], const float (&offset)[4], const bool clockwise);
#endif

#if ENABLED(BEZIER_CURVE_SUPPORT)
  void plan_cubic_move(const float (&cart)[XYZEJH], const float (&offset)[4]);
#endif
...
void sync_plan_position() {
    ...
    #endif
    planner.set_position_mm(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_CART], current_position[J_CART], current_position[H_CART]);
  #endif
}
void sync_plan_position_e() { planner.set_e_position_mm(current_position[E_CART]); }
void sync_plan_position_j() { planner.set_j_position_mm(current_position[J_CART]); }
void sync_plan_position_h() { planner.set_h_position_mm(current_position[H_CART]); }
...
void servo_init() {
	...
	MOVE_SERVO(0, 0);//舵机0初始化位置
  #endif
  #if NUM_SERVOS >= 2 && HAS_SERVO_1
    servo[1].attach(SERVO1_PIN);
    servo[1].detach();
	MOVE_SERVO(1, 100);//舵机1初始化位置
  #endif
...
inline void buffer_line_to_current_position() {
    ...
    planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_CART], current_position[J_CART], current_position[H_CART], feedrate_mm_s, active_extruder);
  #endif
}
...
inline void buffer_line_to_destination(const float &fr_mm_s) {
    ...
    UNUSED(fr_mm_s);
  #else
    planner.buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_CART], destination[J_CART], destination[H_CART], fr_mm_s, active_extruder);
  #endif
}
...
static void do_homing_move(const AxisEnum axis, const float distance, const float fr_mm_s=0) {
  ...
  // Do the move, which is required to hit an endstop
    ...
    planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_CART], current_position[J_CART], current_position[H_CART], fr_mm_s ? fr_mm_s : homing_feedrate(axis), active_extruder);
  #endif
...
void gcode_get_destination() {
  LOOP_XYZEJH(i) {
    if (parser.seen(axis_codes[i])) {
	  ...
	  destination[i] = (axis_relative_modes[i] || relative_mode)
		  ? current_position[i] + v
		  : (i == J_CART) ? v : LOGICAL_TO_NATIVE(v, i);

	  destination[i] = (axis_relative_modes[i] || relative_mode)
		  ? current_position[i] + v
		  : (i == H_CART) ? v : LOGICAL_TO_NATIVE(v, i);
...
inline void gcode_G92() {
	if (parser.linearval('E'))
		idle();
	else if (parser.linearval('J'))
		idle();
	else if (parser.linearval('H'))
		idle();
	...
	for (int8_t i = 0; i < NUM_AXIS; i++) {
		if (parser.seenval(axis_codes[i])) {
			if (i == E_AXIS) {
				...
				sync_plan_position_j();
				sync_plan_position_h();
			}
			else {
#ifdef SCARA
...
#else
				current_position[i] = parser.value_axis_units((AxisEnum)i);// + home_offset[XYZ];
#endif
				planner.set_position_mm(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], current_position[J_AXIS], current_position[H_AXIS]);
			}
		}
	}
/*
  ...
  else if (didJ) sync_plan_position_j();
  else if (didH) sync_plan_position_h();
...
inline void gcode_M17() {
  ...
  inline void gcode_M20() {
	...
	//enqueue_and_echo_commands_P(PSTR("M280 P0 S100"));   //手动发送控制命令代码（命令：M280）
  }
...
inline void gcode_M82() { axis_relative_modes[E_CART] = false;
						  axis_relative_modes[J_CART] = false;
						  axis_relative_modes[H_CART] = false;
						}
...
inline void gcode_M83() { axis_relative_modes[E_CART] = true;
						  axis_relative_modes[J_CART] = true;
						  axis_relative_modes[H_CART] = true;
						}
...
inline void gcode_M18_M84() {
  if (parser.seenval('S')) {
    ...
    bool all_axis = !(parser.seen('X') || parser.seen('Y') || parser.seen('Z') || parser.seen('E') || parser.seen('J') || parser.seen('H'));
    if (all_axis) {
      planner.finish_and_disable();
    }
    else {
      ...
      if (parser.seen('Z')) disable_Z();
		...
		if (parser.seen('J')) disable_j_steppers();
		if (parser.seen('H')) disable_h_steppers();
      #endif
    }
...
inline void gcode_M92() {
  ...
  LOOP_NUM_AXIS(i) {
    if (parser.seen(RAW_AXIS_CODES(i))) {
	  ...
	  if (i == J_AXIS) {
		  const float value = parser.value_per_axis_unit((AxisEnum)(J_AXIS + TARGET_EXTRUDER));
		  if (value < 20) {
			  const float factor = planner.axis_steps_per_mm[J_AXIS + TARGET_EXTRUDER] / value; // increase e constants if M92 E14 is given for netfab.
		#if DISABLED(JUNCTION_DEVIATION)
			  planner.max_jerk[J_AXIS] *= factor;
		#endif
			  planner.max_feedrate_mm_s[J_AXIS + TARGET_EXTRUDER] *= factor;
			  planner.max_acceleration_steps_per_s2[J_AXIS + TARGET_EXTRUDER] *= factor;
	      }
		  planner.axis_steps_per_mm[J_AXIS + TARGET_EXTRUDER] = value;
        }

	  if (i == H_AXIS) {
		  const float value = parser.value_per_axis_unit((AxisEnum)(H_AXIS + TARGET_EXTRUDER));
		  if (value < 20) {
			  const float factor = planner.axis_steps_per_mm[H_AXIS + TARGET_EXTRUDER] / value; // increase e constants if M92 E14 is given for netfab.
		#if DISABLED(JUNCTION_DEVIATION)
			  planner.max_jerk[H_AXIS] *= factor;
		#endif
			  planner.max_feedrate_mm_s[H_AXIS + TARGET_EXTRUDER] *= factor;
			  planner.max_acceleration_steps_per_s2[H_AXIS + TARGET_EXTRUDER] *= factor;
		  }
		  planner.axis_steps_per_mm[H_AXIS + TARGET_EXTRUDER] = value;
	  }
...
void report_current_position() {
  ...
  SERIAL_PROTOCOLPAIR(" J:", current_position[J_CART]);
  SERIAL_PROTOCOLPAIR(" H:", current_position[H_CART]);
...
inline void gcode_M205() {
  ...
  if (parser.seen('T')) planner.min_travel_feedrate_mm_s = parser.value_linear_units();
	...
	if (parser.seen('H')) planner.max_jerk[H_AXIS] = parser.value_linear_units();
  #endif
}
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
		...
		case 2: setup(); break;                                   // M2: 初始化各部件，刷新串口
...
/*********************对板卡上三个场效应管的开关进行控制*************************/
        ...
        //D8口的场效应管控制
		case 6: digitalWrite(8, HIGH); delay(300); break;                     // M6:将D8引脚设为高电平
	    case 7: digitalWrite(8, LOW); delay(300); break;                      // M7:将D8引脚设为低电平

        //D9口的场效应管控制
	    case 8: digitalWrite(9, HIGH); delay(300); break;                     // M8:将D9引脚设为高电平
        case 9: digitalWrite(9, LOW); delay(300); break;                      // M9:将D9引脚设为低电平

	    //D10口的场效应管控制
	    case 10: digitalWrite(10, HIGH); break;                   // M10:将D10引脚设为高电平
        case 11: digitalWrite(10, LOW); break;                    // M11:将D10引脚设为低电平
...
void prepare_move_to_destination() {
   ...
   */
  void plan_arc(
    const float (&cart)[XYZEJH], // Destination position
    const float (&offset)[4], // Center of rotation relative to current_position
	...
	raw[J_CART] = current_position[J_CART];
	raw[H_CART] = current_position[H_CART];
...
void disable_j_stepper(const uint8_t j) 
{
	switch (j) {
	case 0: disable_J0(); break;
	}
}

void disable_h_stepper(const uint8_t h)
{
	switch (h) {
	case 0: disable_H0(); break;
	}
}

void disable_j_steppers()
{
	disable_J0();
}

void disable_h_steppers()
{
	disable_H0();
}
...
void setup() {
  ...
  lcd_reset_status();
    ...
    card.beginautostart();
  #endif
  //enqueue_and_echo_commands_P(PSTR("M21"));   //手动发送控制命令代码（命令：M21）
