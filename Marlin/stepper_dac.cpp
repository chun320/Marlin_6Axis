...
#include "Marlin.h"
  ...
  uint8_t dac_channel_pct[XYZEJH] = DAC_MOTOR_CURRENT_DEFAULT;
  ...
  void dac_current_set_percents(const uint8_t pct[XYZEJH]) {
  ...
  void dac_print_values() {
	...
	SERIAL_ECHOPAIR(") J:", dac_perc(J_AXIS));
	SERIAL_ECHOPAIR(" (", dac_amps(J_AXIS));
	SERIAL_ECHOPAIR(") H:", dac_perc(H_AXIS));
	SERIAL_ECHOPAIR(" (", dac_amps(H_AXIS));
    SERIAL_ECHOLNPGM(")");
  }
