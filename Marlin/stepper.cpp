 ...
 */
  ...
  #define J_APPLY_STEP(v,Q) J_STEP_WRITE(active_extruder, v)
  #define H_APPLY_STEP(v,Q) H_STEP_WRITE(active_extruder, v)
...
void Stepper::set_directions() {
  ...
  #define SET_STEP_DIR(A) \
    ...
    SET_STEP_DIR(D);
	  ...
	  if (motor_direction(H_AXIS)) {
		  REV_H_DIR(active_extruder);
		  count_direction[H_AXIS] = -1;
	  }
	  else {
		  NORM_H_DIR(active_extruder);
		  count_direction[H_AXIS] = 1;
	  }

	  if (motor_direction(J_AXIS)) {
		  REV_J_DIR(active_extruder);
		  count_direction[J_AXIS] = -1;
	  }
	  else {
		  NORM_J_DIR(active_extruder);
		  count_direction[J_AXIS] = 1;
	  }
    ...
    if ((current_block = planner.get_current_block())) {
      ...
      while (TEST(current_block->flag, BLOCK_BIT_SYNC_POSITION)) {
        _set_position(
          ...
          #endif
          current_block->position[E_AXIS], current_block->position[J_AXIS], current_block->position[H_AXIS]
        );
        planner.discard_current_block();
      ...
      delta_error[X_AXIS] = delta_error[Y_AXIS] = delta_error[Z_AXIS] = delta_error[E_AXIS] = delta_error[J_AXIS] = delta_error[H_AXIS] = -int32_t(step_event_count);
	  ...
	  advance_dividend[H_AXIS] = current_block->steps[H_AXIS] << 1;
...
void Stepper::init() {
...
#if HAS_J_ENABLE
	J0_ENABLE_INIT;
	if (!J_ENABLE_ON) J0_ENABLE_WRITE(HIGH);
#endif

#if HAS_H_ENABLE
	H0_ENABLE_INIT;
	if (!H_ENABLE_ON) H0_ENABLE_WRITE(HIGH);
#endif
  ...
  #define J_AXIS_INIT(NUM) AXIS_INIT(J## NUM, J)
  #define H_AXIS_INIT(NUM) AXIS_INIT(H## NUM, H)
...
#if J_STEPPERS > 0 && HAS_J_STEP
	J_AXIS_INIT(0);
#endif

#if H_STEPPERS > 0 && HAS_H_STEP
	H_AXIS_INIT(0);
#endif
...
void Stepper::_set_position(const int32_t &a, const int32_t &b, const int32_t &c,
      ...
      const int32_t &d,
    #endif
  const int32_t &e, const int32_t &j, const int32_t &h
) {
  #if CORE_IS_XY
    // corexy positioning
 ...
 */
  ...
  void Stepper::microstep_init() {
	...
	#if HAS_H_MICROSTEPS
	  SET_OUTPUT(H_MS1_PIN);
	  SET_OUTPUT(H_MS2_PIN);
	#endif
