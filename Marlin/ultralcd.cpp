...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void _lcd_move_j() {
	  ...
	  if (encoderPosition) {
		  if (!processing_manual_move) {
			  ...
			  manual_move_offset += diff;
			#else
			  current_position[J_CART] += diff;
			#endif
			  manual_move_to_current(J_AXIS
			#if E_MANUAL > 1
				  , eindex
			#endif
			  );
	...
	#endif // E_MANUAL > 1
		  lcd_implementation_drawedit(pos_label, ftostr41sign(current_position[J_CART]
#if IS_KINEMATIC
			  + manual_move_offset
#endif
		  ));
  ...
  void _lcd_move_h() 
  {
	  ...
	  if (encoderPosition) {
		  if (!processing_manual_move) {
			  ...
			  manual_move_offset += diff;
#else
			  current_position[H_CART] += diff;
#endif
			  manual_move_to_current(H_AXIS
#if H_MANUAL > 1
				  , eindex
#endif
			  );
...
#endif // E_MANUAL > 1
		  lcd_implementation_drawedit(pos_label, ftostr41sign(current_position[H_CART]
#if IS_KINEMATIC
			  + manual_move_offset
#endif
		  ));
  ...
  void lcd_move_j() { _lcd_move_j(); }
  void lcd_move_h() { _lcd_move_h(); }
  ...
  void _lcd_move_distance_menu(const AxisEnum axis, const screenFunc_t func) {
    ...
    if (LCD_HEIGHT >= 4) {
      switch (axis) {
        ...
        default:
          STATIC_ITEM(MSG_MOVE_H, true, true); break;
      }
    }
  ...
  void lcd_move_get_j_amount()        { _lcd_move_distance_menu(J_AXIS, lcd_move_j); }
  void lcd_move_get_h_amount()		  { _lcd_move_distance_menu(H_AXIS, lcd_move_h); }
  ...
  void lcd_move_menu() {
    ...
    if (_MOVE_XYZ_ALLOWED) {
	  ...
	  MENU_ITEM(submenu, MSG_MOVE_H, lcd_move_get_h_amount);
  ...
  void lcd_control_temperature_menu() {
    ...
    void lcd_control_motion_velocity_menu() {
      ...
      MENU_MULTIPLIER_ITEM_EDIT(float3, MSG_VMAX MSG_C, &planner.max_feedrate_mm_s[C_AXIS], 1, 999);
		...
		MENU_MULTIPLIER_ITEM_EDIT(float3, MSG_VMAX MSG_J, &planner.max_feedrate_mm_s[J_AXIS], 1, 999);
		MENU_MULTIPLIER_ITEM_EDIT(float3, MSG_VMAX MSG_H, &planner.max_feedrate_mm_s[H_AXIS], 1, 999);
    ...
    void lcd_control_motion_acceleration_menu() {
      ...
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_C, &planner.max_acceleration_mm_per_s2[C_AXIS], 10, 99000, _reset_acceleration_rates);
		...
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_J, &planner.max_acceleration_mm_per_s2[J_AXIS], 100, 99000, _reset_acceleration_rates);
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_H, &planner.max_acceleration_mm_per_s2[H_AXIS], 100, 99000, _reset_acceleration_rates);
    ...
    void lcd_control_motion_jerk_menu() {
      ...
      MENU_BACK(MSG_MOTION);
		...
		MENU_MULTIPLIER_ITEM_EDIT(float3, MSG_VJ_JERK, &planner.max_jerk[J_AXIS], 1, 990);
		MENU_MULTIPLIER_ITEM_EDIT(float3, MSG_VH_JERK, &planner.max_jerk[H_AXIS], 1, 990);
      #endif
      END_MENU();
    }
    ...
    void lcd_control_motion_steps_per_mm_menu() {
      ...
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float62, MSG_CSTEPS, &planner.axis_steps_per_mm[C_AXIS], 5, 9999, _planner_refresh_positioning);
		...
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float62, MSG_JSTEPS, &planner.axis_steps_per_mm[J_AXIS], 5, 9999, _planner_refresh_positioning);
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float62, MSG_HSTEPS, &planner.axis_steps_per_mm[H_AXIS], 5, 9999, _planner_refresh_positioning);
