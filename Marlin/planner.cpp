...
float Planner::max_feedrate_mm_s[NUM_AXIS_N], // (mm/s) M203 XYZE - Max speeds
  ...
  #define _JINDEX J_AXIS
  #define _HINDEX H_AXIS
...
float Planner::j_factor[EXTRUDERS] = ARRAY_BY_EXTRUDERS1(1.0f); // The flow percentage and volumetric multiplier combine to scale E movement
float Planner::h_factor[EXTRUDERS] = ARRAY_BY_EXTRUDERS1(1.0f); // The flow percentage and volumetric multiplier combine to scale E movement
...
int32_t Planner::position[NUM_AXIS] = { 0, 0, 0, 0, 0, 0 };
...
void Planner::recalculate() {
  ...
  void Planner::getHighESpeed() {
    ...
    for (uint8_t b = block_buffer_tail; b != block_buffer_head; b = next_block_index(b)) {
      ...
      if (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS] || block->steps[J_AXIS] || block->steps[H_AXIS])
	  {
        const float se = (float)block->steps[E_AXIS] / block->step_event_count * SQRT(block->nominal_speed_sqr); // mm/sec;
        NOLESS(high, se);
      }
	  else if (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS] || block->steps[E_AXIS] || block->steps[H_AXIS])
	  {
		  const float sj = (float)block->steps[J_AXIS] / block->step_event_count * SQRT(block->nominal_speed_sqr); // mm/sec;
		  NOLESS(high, sj);
	  }
	  else if (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS] || block->steps[J_AXIS] || block->steps[E_AXIS])
	  {
		  const float sh = (float)block->steps[H_AXIS] / block->step_event_count * SQRT(block->nominal_speed_sqr); // mm/sec;
		  NOLESS(high, sh);
	  }
    }
...
void Planner::check_axes_activity() {
  ...
  if (has_blocks_queued()) {
    ...
    for (uint8_t b = block_buffer_tail; b != block_buffer_head; b = next_block_index(b)) {
      ...
      LOOP_XYZEJH(i) if (block->steps[i]) axis_active[i]++;
    }
  }
  else {
    #if FAN_COUNT > 0
	...
	if (!axis_active[J_AXIS]) disable_j_steppers();
#endif
#if ENABLED(DISABLE_H)
	if (!axis_active[H_AXIS]) disable_h_steppers();
#endif
...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  int32_t dj = target[J_AXIS] - position[J_AXIS];
  int32_t dh = target[H_AXIS] - position[H_AXIS];
  ...
  if (dh < 0) SBI(dm, H_AXIS);

  const float esteps_float = de * e_factor[extruder];
  const uint32_t esteps = ABS(esteps_float) + 0.5f;

  const float jsteps_float = dj * j_factor[extruder];
  const uint32_t jsteps = ABS(jsteps_float) + 0.5f;

  const float hsteps_float = dh * h_factor[extruder];
  const uint32_t hsteps = ABS(hsteps_float) + 0.5f;
  ...
  block->steps[J_AXIS] = jsteps;
  block->steps[H_AXIS] = hsteps;

  block->step_event_count = (
    #if ENABLED(HANGPRINTER)
      MAX5(block->steps[A_AXIS], block->steps[B_AXIS], block->steps[C_AXIS], block->steps[D_AXIS], esteps)
    #else
      MAX6(block->steps[A_AXIS], block->steps[B_AXIS], block->steps[C_AXIS], block->steps[E_AXIS], block->steps[J_AXIS], hsteps)
    #endif
  );
  ...
  // Enable active axes
	...
	if (block->steps[J_AXIS]) enable_J0();
	if (block->steps[H_AXIS]) enable_H0();
  ...
  delta_mm[J_AXIS] = jsteps_float * steps_to_mm[J_AXIS_N];
  delta_mm[H_AXIS] = hsteps_float * steps_to_mm[H_AXIS_N];

  if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[J_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[H_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
    block->millimeters = ABS(delta_mm[E_AXIS]);
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[E_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[H_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = ABS(delta_mm[J_AXIS]);
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[E_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[J_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = ABS(delta_mm[H_AXIS]);
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[H_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = SQRT(sq(delta_mm[E_AXIS]) + sq(delta_mm[J_AXIS]));
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[J_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = SQRT(sq(delta_mm[E_AXIS]) + sq(delta_mm[H_AXIS]));
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT&& block->steps[E_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = SQRT(sq(delta_mm[J_AXIS]) + sq(delta_mm[H_AXIS]));
  }
  else if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT)
  {
	  block->millimeters = SQRT(sq(delta_mm[E_AXIS]) + sq(delta_mm[J_AXIS]) + sq(delta_mm[H_AXIS]));
  }
  else if (!millimeters) {
  ...
  if (!block->steps[A_AXIS] && !block->steps[B_AXIS] && !block->steps[C_AXIS]
    ...
    if (block->step_event_count <= cutoff_long) {
	  ...
	  LIMIT_ACCEL_LONG(J_AXIS, ACCEL_IDX);
	  LIMIT_ACCEL_LONG(H_AXIS, ACCEL_IDX);
	  ...
	  LIMIT_ACCEL_FLOAT(J_AXIS, ACCEL_IDX);
	  LIMIT_ACCEL_FLOAT(H_AXIS, ACCEL_IDX);
...
void Planner::buffer_sync_block() {
  ...
  block->position[H_AXIS] = position[H_AXIS];
...
bool Planner::buffer_segment(const float &a, const float &b, const float &c
    ...
    , const float &d
  #endif
  , const float &e, const float &j, const float &h, const float &fr_mm_s, const uint8_t extruder, const float &millimeters/*=0.0*/
  #if ENABLED(UNREGISTERED_MOVE_SUPPORT)
    , bool count_it /* = true */
  #endif
) {
  ...
  const int32_t target[NUM_AXIS] = {
	...
	LROUND(j * axis_steps_per_mm[J_AXIS_N]),
	LROUND(h * axis_steps_per_mm[H_AXIS_N])
  ...
  if (DEBUGGING(DRYRUN)) {
    if (COUNT_MOVE) {
	  ...
	  position[J_AXIS] = target[J_AXIS];
	  position[H_AXIS] = target[H_AXIS];
      #if HAS_POSITION_FLOAT
        position_float[E_AXIS] = e;
		position_float[J_AXIS] = j;
		position_float[H_AXIS] = h;
      #endif
    }
...
void Planner::_set_position_mm(const float &a, const float &b, const float &c
    ...
    , const float &d
  #endif
  , const float &e, const float &j, const float &h
) {
  #if ENABLED(DISTINCT_E_FACTORS)
  ...
  position[J_AXIS] = LROUND(j * axis_steps_per_mm[_JINDEX]);
  position[H_AXIS] = LROUND(h * axis_steps_per_mm[_HINDEX]);
...
void Planner::set_position_mm_kinematic(const float (&cart)[XYZEJH]) {
  #if PLANNER_LEVELING
    ...
    float raw[XYZ] = { cart[X_AXIS], cart[Y_AXIS], cart[Z_AXIS] };
  #else
    const float (&raw)[XYZEJH] = cart;
  #endif
  #if IS_KINEMATIC
    inverse_kinematics(raw);
    #if ENABLED(HANGPRINTER)
      _set_position_mm(line_lengths[A_AXIS], line_lengths[B_AXIS], line_lengths[C_AXIS], line_lengths[D_AXIS], cart[E_CART], cart[J_CART], cart[H_CART]);
    #else
      _set_position_mm(delta[A_AXIS], delta[B_AXIS], delta[C_AXIS], cart[E_CART]], cart[J_CART]], cart[H_CART]);
    #endif
  #else
    _set_position_mm(raw[X_AXIS], raw[Y_AXIS], raw[Z_AXIS], cart[E_CART], cart[J_CART], cart[H_CART]);
  #endif
}
